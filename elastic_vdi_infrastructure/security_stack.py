...
class SecurityStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str,
        ...
        self.efadmin_password = secretsmanager.Secret(self, "EfadminPassword",
        ...
        # Create ALB SG
        self.alb_security_group, self.broker_alb_security_group = self.create_alb_sg(vpc, config)       
        ...
        self.ef_security_group, self.dcv_security_group, self.broker_security_group = self.create_nodes_security_groups(
            ...
            self.broker_alb_security_group
        )
    ...
    def create_alb_sg(
        ...
        broker_alb_security_group = ec2.SecurityGroup(self, "ALBSecurityGroupBroker",
                                               vpc=vpc,
    ...
    def create_nodes_security_groups(
        ...
        broker_security_group = ec2.SecurityGroup(self, "BrokerSecurityGroup",
                                              vpc=vpc,
        ...
        broker_security_group.add_ingress_rule(
            broker_alb_security_group, ec2.Port.tcp(8445), "allow broker access ")
        broker_security_group.add_ingress_rule(
            broker_alb_security_group, ec2.Port.tcp(8443), "allow broker access ")
        broker_security_group.add_ingress_rule(
            broker_security_group, ec2.Port.tcp_range(47100,47200), "allow broker to broker ")
        broker_security_group.add_ingress_rule(
            broker_security_group, ec2.Port.tcp_range(47500,47600), "allow broker to broker ")
    ...
    def create_storage_security_group(
        ...
        storage_security_group = ec2.SecurityGroup(self, "StorageSecurityGroup",
                                              vpc=vpc,
        ...
        storage_security_group.add_ingress_rule(
            dcv_security_group, ec2.Port.tcp(111), "Remote procedure call for NFS - DCV nodes")
        storage_security_group.add_ingress_rule(
            ef_security_group, ec2.Port.tcp(111), "Remote procedure call for NFS - EF node")

        storage_security_group.add_ingress_rule(
            dcv_security_group, ec2.Port.udp(111), "Remote procedure call for NFS - DCV nodes")
        storage_security_group.add_ingress_rule(
            ef_security_group, ec2.Port.udp(111), "Remote procedure call for NFS - EF node")     
  
        storage_security_group.add_ingress_rule(
            dcv_security_group, ec2.Port.tcp(2049), "NFS server daemon - DCV nodes")
        storage_security_group.add_ingress_rule(
            ef_security_group, ec2.Port.tcp(2049), "NFS server daemon - EF node")     
          
        storage_security_group.add_ingress_rule(
            dcv_security_group, ec2.Port.udp(2049), "NFS server daemon - DCV nodes")
        storage_security_group.add_ingress_rule(
            ef_security_group, ec2.Port.udp(2049), "NFS server daemon - EF node")
            
            
        storage_security_group.add_ingress_rule(
            dcv_security_group, ec2.Port.tcp_range(20001,20003), "NFS mount, status monitor, and lock daemon - DCV nodes")
        storage_security_group.add_ingress_rule(
            ef_security_group, ec2.Port.tcp_range(20001,20003), "NFS mount, status monitor, and lock daemon - EF node")     
          
        storage_security_group.add_ingress_rule(
            dcv_security_group, ec2.Port.udp_range(20001,20003), "NFS mount, status monitor, and lock daemon - DCV nodes")
        storage_security_group.add_ingress_rule(
            ef_security_group, ec2.Port.udp_range(20001,20003), "NFS mount, status monitor, and lock daemon - EF node")  
    ...
    def create_ef_role(
        ...
        role_ef.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=["arn:aws:ssm:" + config['region'] + ":" + config['account'] + ":parameter/dcv/*",
                           "arn:aws:ssm:" + config['region'] + ":" + config['account'] + ":parameter/dcvbroker/*"],
            )
        )
        ...
        role_ef.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=["arn:aws:s3:::" + closing_hook.s3_bucket_name + "/" + closing_hook.s3_object_key,
                           "arn:aws:s3:::" + starting_hook.s3_bucket_name + "/" + starting_hook.s3_object_key,
                           "arn:aws:s3:::" + interactive_builtin_linux_desktop.s3_bucket_name + "/" + interactive_builtin_linux_desktop.s3_object_key,
                           "arn:aws:s3:::" + interactive_builtin_windows_desktop.s3_bucket_name + "/" + interactive_builtin_windows_desktop.s3_object_key,
                           "arn:aws:s3:::" + swagger_client.s3_bucket_name + "/" + swagger_client.s3_object_key,
                           "arn:aws:s3:::" + dcvasg_cloudwatch_metrics.s3_bucket_name + "/" + dcvasg_cloudwatch_metrics.s3_object_key,
                           "arn:aws:s3:::" + dcvasg_triggers.s3_bucket_name + "/" + dcvasg_triggers.s3_object_key,
                           "arn:aws:s3:::" + dcvasg.s3_bucket_name + "/" + dcvasg.s3_object_key,
                           "arn:aws:s3:::" + dcvsm.s3_bucket_name + "/" + dcvsm.s3_object_key,
                           "arn:aws:s3:::" + grid_list_hosts_ui.s3_bucket_name + "/" + grid_list_hosts_ui.s3_object_key,
                           "arn:aws:s3:::" + enginframe_node_configuration.s3_bucket_name + "/" + enginframe_node_configuration.s3_object_key,
                           "arn:aws:s3:::" + imds_access.s3_bucket_name + "/" + imds_access.s3_object_key,
                           "arn:aws:s3:::" + aws_py.s3_bucket_name + "/" + aws_py.s3_object_key],
    ...
    def create_dcv_role(
        ...
        role_dcv.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=["arn:aws:s3:::" + linux_userdata.s3_bucket_name + "/" + linux_userdata.s3_object_key,
                           "arn:aws:s3:::" + windows_userdata.s3_bucket_name + "/" + windows_userdata.s3_object_key,
                           "arn:aws:s3:::dcv-license." + config['region'] + "/*",
                           "arn:aws:s3:::" + imds_access.s3_bucket_name + "/" + imds_access.s3_object_key],
            )
        )
    ...
    def create_broker_role(
        ...
        role_broker = iam.Role(
            self, "Broker_ROLE", assumed_by=iam.ServicePrincipal("ec2.amazonaws.com"))
        # Allow console access with SSM
        role_broker.add_managed_policy(iam.ManagedPolicy.from_aws_managed_policy_name(
            "AmazonSSMManagedInstanceCore"))
            
        
        # Allow to describe the instances and elasticloadbalancing health targets
        ...
        role_broker.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=["arn:aws:ssm:" + config['region'] + ":" + config['account'] + ":parameter/dcvbroker/*"],
            )
        )
        ...
        role_broker.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=["arn:aws:s3:::" + broker_userdata.s3_bucket_name + "/" + broker_userdata.s3_object_key],
            )
        )
    ...
    def create_lambda_role(
        ...
        lambda_role.add_to_policy(
            iam.PolicyStatement(
                ...
                resources=["arn:aws:ssm:" + config['region'] + ":" + config['account'] + ":parameter/dcv/linux/DcvBrokerCACertificate"],
            )
        )
