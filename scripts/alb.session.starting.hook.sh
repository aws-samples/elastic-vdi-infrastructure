...
main() {
    ...
    if [ -z "$_target_group_arn" ]; then
        ...
        local -- _priority
        _priority=$((_current_priority+1))
        
        local -- _target_path
        _target_path="$_target_web_url_path*"
        
        local -- _rule_arn
        _rule_arn=$(aws elbv2 create-rule --listener-arn "$_listener_arn" --priority "$_priority" \
    ...
    aws ec2 create-tags --resources "${instance_id}" --tags Key=UserID,Value="${EF_USER}"
